...
from .status import *
from .validators import OrderCheckAndPayment



class ClickUzMerchantAPIView(APIView):
    authentication_classes = []
    serializer_class = ClickUzSerializer
    ...
    def post(self, request):
        ...
        if click_authorization(**data) is False:
            return Response({
        ...
        print()
        print("Click Uz valid: ", self.VALIDATE_CLASS().check_order(order_id=merchant_trans_id, amount=amount))
        print()
        check_order = self.VALIDATE_CLASS().check_order(order_id=merchant_trans_id, amount=amount)
        if check_order is True:
            result = METHODS[action](**data, response_data=data)
